...
import com.literalura.literalura.api.PeticionAPI;
import com.literalura.literalura.model.Autor;
import com.literalura.literalura.model.Idioma;
import com.literalura.literalura.model.Libro;
import com.literalura.literalura.model.LibroRecord;
import com.literalura.literalura.util.JsonParser;
...
public class MenuService {
    ...
    public void guardarLibro() {
        ...
        for (int i = 0; i < librosObtenidos.size(); i++) {
            ...
            if (!librosObtenidos.get(i).idiomas().isEmpty()) {
                idioma = librosObtenidos.get(i).idiomas().get(0);
            }

            if (!librosObtenidos.get(i).autores().isEmpty()) {
                autor = librosObtenidos.get(i).autores().get(0).nombre();
            }

            System.out.println(
                    (i + 1) + " - " +
                            librosObtenidos.get(i).titulo() + " - " +
                            idioma + " - " +
                            autor
            );
        }
        ...
        if (!libroRecord.autores().isEmpty()) {
            nombreAutor = libroRecord.autores().get(0).nombre();
        }

        Optional<Libro> libroTraidoDelRepo =
                libroService.obtenerLibroPorNombre(libroRecord.titulo());

        Optional<Autor> autorTraidodelRepo =
                autorService.obtenerAutorPorNombre(nombreAutor);
